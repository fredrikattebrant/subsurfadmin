...
public class ImportClient {
	 ...
	 * @throws Exception
	 *             if error is encountered, such as bad permissions
	 */
	 ...
	 * @param input
	 *            the input string for the cell
	 * @throws Exception
	 *             if error is encountered, such as bad permissions
	 */
	public void insertCellEntry(String spreadsheet, String worksheet, int row,
			int column, String input) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		String password = promptForString("Enter password:", "Google docs password");
		...
		SpreadsheetEntry spreadSheetEntry = client.getSpreadsheet(spreadsheet);
		System.out.println("Title: " + spreadSheetEntry.getTitle().getPlainText());

		List<WorksheetEntry> workSheets = spreadSheetEntry.getWorksheets();
		for (WorksheetEntry worksheetEntry : workSheets) {
			String wsTitle =  worksheetEntry.getTitle().getPlainText();
			if (!wsTitle.equals("Medlemmar")) {
				System.out.println("Wrong spreadsheet - aborting");
				System.exit(1);
			}

			int rows = worksheetEntry.getRowCount();
			int cols = worksheetEntry.getColCount();
			
			System.out.println("Rows: " + rows);
			System.out.println("Cols: " + cols);
			
			URL listFeedUrl = worksheetEntry.getListFeedUrl();
			ListFeed feed = client.getService().getFeed(listFeedUrl, ListFeed.class);
...
//					client.insertCellEntry(spreadsheet, worksheet, row + 1,
//							col + 1, cells[col]);
//				}
//
//				// Advance the loop
...
//		}
	...
	public static String promptForString(String prompt, String title) {
		String response = null;
		JPasswordField pwd = new JPasswordField(10);
		int action = JOptionPane.showConfirmDialog(null, pwd, title,
				JOptionPane.OK_CANCEL_OPTION);
		if (action < 0) {
			JOptionPane.showMessageDialog(null,
					"Cancel, X or escape key selected");
		} else {
			response = new String(pwd.getPassword());
		}
		return response;
	}
}
